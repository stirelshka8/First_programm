...
def document_search(search_location):
...
def document_location(search_location):
    ...
    for rack, val_ in search_location.items():
        ...
        if parameter in val_:
            # Переменной присваивается значение ключа словаря
            output_data = rack
            # Выводим полученные данные
            print(f"Документ с номером {parameter} лежит на полке № {output_data}")
            # В случае отсутствия искомых данных переменная будет иметь параметр False
    if output_data == False:
        # Выводим о неудаче
        print(f"Документ с номером {parameter} в базе не найден!")
...
def whole_dictionary(search_location):
    ...
    for docos in search_location:
        ...
        for val_ in docos.values():
            ...
            wh_dict.append(val_)
        # Выводим данные
        print(wh_dict[i], wh_dict[i + 1], wh_dict[i + 2] )
...
def adding_document(search_location_documents, search_location_directories):
    ...
    if new_docos_place in search_location_directories:
        ...
        print(f"Данные нового документа ({new_docos_type} {new_docos_number} {new_docos_name}) занесены!")
    else:
        # Выводим данный текст в случае если указанной полки не существуеит
        print(f"""Добавить данные на указанную Вами полку невозможно, полки не существует.
...
def deleting_document(search_location_documents, search_location_directories):
    ...
    for docos in copy_location_documents:
        ...
        if del_docos_number == docos["number"]:
            ...
            # выводим результат
            print(f"Документ с № {del_docos_number} удален из каталога.")
    ...
    for keys_, val_ in copy_location_directories.items():
        if del_docos_number in val_:
            ...
            print(f"Данные документа с № {del_docos_number} удален с полки № {keys_}")
    # В случае отсутствия искомогообъекта выводим текст 
    if output_data == False:
        print(f"Данные документа с № {del_docos_number} не найден(ы).")
    return
    ...
    for keys_former, val_ in copy_location_directories.items():
        ...
        if mov_docos_number in val_:
            ...
            if new_place in copy_location_directories:
                ...
                print(f"Документ с № {mov_docos_number} перемещен с полки № {keys_former} на полку № {new_place}. ")
                break
            else:
                print(f"Полки с № {new_place} не существует.")
    else:
        print(f"Данные документа с № {mov_docos_number} не найден(ы).")
    return
...
def add_place(search_location_directories):
    ...
    if add_place_number in search_location_directories:
        print(f"Полка с № {add_place_number} уже существует.")
    # Если нет то создаем
    else:
        search_location_directories[add_place_number] = []
        print(f"Полка с № {add_place_number} создана.")
    return
